...
// I18N_LANG_ARRAY,
// type I18N_LANG_KIND,
HTML_TAG_END__EN_US, HTML_TAG_END__ZH_CN, HTML_TAG_END__ZH_TW, I18N_HTML_BEGIN_TAG_LENGTH, joinPath, LF, mkdirSync, readTextFileSync, SEP, SEPARATOR_OF_SPLIT, statSync, writeTextFileSync, } from 'https://raw.githubusercontent.com/anqisoft/ts_utils/main/index.ts';
...
async function translateByGoogleCore(from, langFrom, langTo) {
    ...
    try {
        ...
        if (FROM_LENGTH <= TRANSLATE_MAX_CHAR_COUNT_PER_TIME) {
            ...
            const END_TAG = `</${langFrom === GOOGLE_TRANSLATE_LANG_EN
                ? 'en_us'
...
async function splitCommentCore(sourceFilename, commentFilesPath) {
        ...
        if (!existsSync(OTHER_FILENAME)) {
            switch (index) {
                case 0:
                    ...
                    writeTextFileSync(OTHER_FILENAME, (await translateByGoogleCore(EN_BEFORE_TRANSLATE, GOOGLE_TRANSLATE_LANG_CN, GOOGLE_TRANSLATE_LANG_EN))
                        .replaceAll(HTML_TAG_BEGIN__ZH_CN, HTML_TAG_BEGIN__EN_US)
                        .replaceAll(HTML_TAG_END__ZH_CN, HTML_TAG_END__EN_US)
                        .replace(EN_REPLACE_PATCH_FROM, EN_REPLACE_PATCH_TO), FILE_CREATE_NEW_AND_MODE_ALL);
                    break;
                case 1:
                ...
                case 2:
                    ...
                    // 	FILE_CREATE_NEW_AND_MODE_ALL,
                    // );
                    writeTextFileSync(OTHER_FILENAME, await translateByGoogleCore(TW_BEFORE_TRANSLATE, GOOGLE_TRANSLATE_LANG_CN, GOOGLE_TRANSLATE_LANG_TW), FILE_CREATE_NEW_AND_MODE_ALL);
                    break;
                default:
                    break;
            }
...
export function joinComments(sourceFilename, commentFilesPath) {
    try {
        ...
        const GOAL_PATH = joinPath(commentFilesPath, sourceFilename.split(SEP).pop(), SEP);
        const US_COMMENTS = getSplitResultFromGoalFileByLang(GOAL_PATH, 'en_us');
        const CN_COMMENTS = getSplitResultFromGoalFileByLang(GOAL_PATH, 'zh_cn');
        const TW_COMMENTS = getSplitResultFromGoalFileByLang(GOAL_PATH, 'zh_tw');
        const CODES_ARRAY = SOURCE_CONTENT.replace(/((\<|\<\/)(en_us|zh_cn|zh_tw)\>)/g, SEPARATOR_OF_SPLIT)
            .split(SEPARATOR_OF_SPLIT);
...
async function cn2trilingualCore(sourceFilename) {
    ...
    const EN_FULL_CONTENT = (await translateByGoogleCore(EN_BEFORE_TRANSLATE, GOOGLE_TRANSLATE_LANG_CN, GOOGLE_TRANSLATE_LANG_EN)).replaceAll(HTML_TAG_BEGIN__ZH_CN, HTML_TAG_BEGIN__EN_US)
        .replaceAll(HTML_TAG_END__ZH_CN, HTML_TAG_END__EN_US)
    ...
    const TW_FULL_CONTENT = await translateByGoogleCore(TW_BEFORE_TRANSLATE, GOOGLE_TRANSLATE_LANG_CN, GOOGLE_TRANSLATE_LANG_TW);
...
export function splitFiles(keepLangTag, sourceFilenames) {
    ...
    for (let i = 0; i < COUNT; ++i) {
        ...
        try {
            ...
            const GOAL_PATH = joinPath(SOURCE_FILENAME_SPLIT_RESULT.join(SEP), 'i18n');
            const splitFile = () => {
                const SOURCE_CONTENT = readTextFileSync(SOURCE_FILENAME);
                [
